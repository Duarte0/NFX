...
def _subject_hash(email: str, ip_address: str) -> str:
    ...
    return hmac.new(key, f"{normalize_email(email)}\x00{ip_address}".encode("utf-8"), hashlib.sha256).hexdigest()


def _event(event: str, *, result: str, actor_id: str | None = None, ip_address: str | None = None) -> None:
    logger.info("identity_event", extra={"event": event, "result": result, "actor_id": actor_id, "ip": ip_address})
...
def authenticate(email: str, password: str, ip_address: str, user_agent: str) -> tuple[str | None, SessionIdentity | None]:
    ...
    LoginThrottle.objects.filter(subject_hash=subject_hash).update(failures=0, next_allowed_at=now)
    ...
    _event("login_success", result="success", actor_id=str(user.id), ip_address=ip_address)
...
def resolve_session(token: str | None, *, touch: bool = True) -> SessionIdentity | None:
    ...
    if session.expires_at <= now or session.revocation_version != session.user.revocation_version:
        IdentitySession.objects.filter(id=session.id, revoked_at__isnull=True).update(revoked_at=now)
        _event("session_expired", result="expired", actor_id=str(session.user.id), ip_address=session.ip_address)
        return None
    if touch:
        ...
        if not updated:
            return None
    return SessionIdentity(str(session.user.id), session.user.email, session.user.name, session.user.role)
...
def revoke_session(token: str | None) -> None:
    if not token:
        return
    now = timezone.now()
    session = IdentitySession.objects.select_related("user").filter(token_hash=_digest(token)).first()
    if session and session.revoked_at is None:
        IdentitySession.objects.filter(id=session.id, revoked_at__isnull=True).update(revoked_at=now)
        _event("logout", result="success", actor_id=str(session.user.id), ip_address=session.ip_address)


def require_authorized(identity: SessionIdentity | None, action: str, owner_id: str | None = None) -> bool:
