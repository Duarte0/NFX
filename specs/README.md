...
Estas 36 specs são o handoff de implementação do MVP e dos follow-ups aprovados. `PRD.md` é autoridade de produto; `ARCHITECTURE.md`, de decisões/invariantes; `IMPLEMENTATION_PLAN.md`, de backlog e sequência; o código existente, da baseline real. P0, P1 e P2 estão concluídos; a higiene do template de ambiente foi verificada no issue 0007 e o contrato reproduzível de `make build` foi concluído no issue 0008. O issue 0011 concluiu a extração arquitetural transversal do frontend sem alterar o status ou o contrato de nenhuma spec P1–P4. O issue 0015 concluiu os slices P7-01/P7-02 de consulta e download individual; o issue 0019 concluiu P8-03 com cálculo-on-read e prévia administrativa; o issue 0023 concluiu P7-03 com renderer pinado, worker, artefatos derivados e UI; o issue 0024 concluiu P9-03 com saga de exclusão controlada e recovery; os issues 0026–0030 concluíram os slices de drill-down de coleta, documentos, empresas, certificados e jobs em P8-02; o issue 0042 concluiu o gate cross-owner do contrato P8-02 sem encerrar a spec parcial; os issues 0032 e 0033 concluíram os follow-ups P0 de entrega do build React e provisionamento seguro do administrador; o issue 0034 concluiu P10-01 com tokens e primitives acessíveis; o issue 0035 concluiu P10-02 com shell e matriz sintética Chrome/Firefox/Edge; o issue 0036 concluiu P10-03 com dashboard agrupado, estados explícitos, retenção stale/error e matriz sintética de browser; o issue 0037 concluiu P10-04 com documentos, filtros, detalhe, ações XML/PDF e estados seguros; o issue 0038 concluiu P10-05 com empresas, certificados, coletas, estados seguros, retenção stale/error e matriz sintética de browser; o issue 0039 concluiu P10-06 com exportações ZIP, estados duráveis, guards de interação, redaction e matriz sintética de browser; o issue 0040 concluiu P10-07 com administração, auditoria, retenção/exclusão, redaction, dialogs críticos e estados de recovery; o issue 0041 concluiu P10-08 com validação transversal de acessibilidade, responsividade e UX. A nova P10-09 é o contrato pendente e pronto para issues que migra a navegação por hash e a entrega da SPA para os requisitos atuais.
...
| # | Progresso/status | Spec | Fase | Backlog exato | Dependências diretas |
|---:|:---:|---|---|---|---|
...
| 2a | [x] histórico; supersedida somente no fallback SPA | [p0-frontend-build-delivery.md](p0-frontend-build-delivery.md) | Follow-up P0 | Build React na rota raiz, MIME, containment, falhas seguras e smoke de imagem concluídos no issue 0032; P10-09 é dono do fallback limitado | Fundação, shell P1 |
...
| 19 | [ ] | [p8-dashboard-and-operational-health.md](p8-dashboard-and-operational-health.md) | P8 | **P8-02 slice inicial (issue 0018) + saúde de backup Admin-only (issue 0025) + drill-down de execuções de coleta (issue 0026) + drill-down de documentos (issue 0027) + drill-down de empresas (issue 0028) + drill-down de certificados (issue 0029) + drill-down de jobs (issue 0030) + gate cross-owner (issue 0042); P5–P7 e disco permanecem indisponíveis/pendentes** | P3-04 + dados P2–P7 disponíveis; entrega progressiva permitida |
...
| 27 | [x] histórico; supersedida somente na navegação | [p10-application-shell.md](p10-application-shell.md) | P10 | P10-02 — shell desktop/notebook e matriz de browser concluídos no issue 0035; P10-09 é dono das rotas SPA | P10-01 |
...
| 33 | [x] | [p10-accessibility-responsive-validation.md](p10-accessibility-responsive-validation.md) | P10 | P10-08 — acessibilidade e validação concluído no issue 0041 | P10-02..07 |
| 34 | [ ] ready for issues | [p10-spa-url-navigation.md](p10-spa-url-navigation.md) | P10 | P10-09 — React Router, rotas URL-endereçáveis, migração de hashes e fallback SPA limitado | P10-01..08; preserva P0 de entrega e owners P1–P9 |

Todos os itens P0-01 a P9-05 e P10-01 a P10-09 estão cobertos; os follow-ups P0 têm owners canônicos separados e foram concluídos nos issues 0032 e 0033. P2-04 é deliberadamente compartilhado pelas duas specs P2 porque a UI integra empresa e certificado; isso é cobertura transversal, não execução duplicada. Nenhum outro backlog tem dois proprietários primários. A seção “Specs futuras” de P3 inclui a spec manual; sua ausência na tabela original do mapa é corrigida neste índice conforme instrução explícita.

As entradas sem marcação de histórico são specs ativas. As duas linhas marcadas como
**histórico; supersedida somente...** preservam evidência de comportamento entregue, mas P10-09
é o contrato canônico para navegação/fallback SPA; elas não devem receber requisitos novos nesse
limite. Não há specs deprecated nem template versionado neste diretório. P5 e P6 usam simuladores
para implementação segura; P5-01/P5-02/P5-03 foram implementados nos issues 0013/0020/0022 e
P6-01/P6-02 no issue 0014. A conexão real continua bloqueada apenas pelas decisões externas
registradas. Os follow-ups P0 foram defeitos confirmados e agora têm evidência própria, sem
reabrir a fundação concluída.
