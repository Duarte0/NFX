...
| P10-02 — shell de aplicação (em validação) | `App.tsx` publica header/sidebar/main, skip link, contexto de sessão, modelo tipado de navegação por papel, active state por hash e destino único autorizado `#certificados`; a composição preserva callbacks, IDs, query strings, drill-downs e autorização server-side | issue 0035; contrato UI renderiza os três papéis e anônimo, verifica landmarks, foco, âncoras, matriz positiva/negativa e ausência de efeitos; lint/build passam sem backend, migration ou dependência nova. Falta a matriz manual Chrome/Firefox/Edge. |
...
| 4 | **P10 Frontend UX & Visual System — in progress (P10-01 concluído; P10-02 em validação)** | Modernizar a experiência operacional sem reescrita funcional: identidade institucional, shell desktop/notebook, componentes compartilhados e apresentação consistente dos fluxos já entregues. Specs P10-01..08 definem os contratos; cada slice só conclui com compatibilidade de contratos e validação dos estados críticos aplicáveis. | É independente de P9-05; o código de P10-02 está pronto, mas seu aceite visual/teclado aguarda browsers. P10-03..08 seguem depois do shell. Preserva `App → features → shared`, contratos HTTP, IDs/âncoras, URLs de drill-down e autorização server-side; não introduzir router, estado global, framework UI ou dependência relevante sem spec e evidência. |
...
| **P10-02 Application Shell — em validação no issue 0035** | Sidebar, header, identidade do usuário, navegação, landmarks, skip link e composição principal para desktop/notebook, com preservação das áreas, hashes, callbacks e visibilidade por papel existentes. | P10-01 comprovado. Falta executar a matriz Chrome/Firefox/Edge em 1024/1280/1440 px; P10-03..08 aguardam o fechamento. Router e estado global continuam fora do escopo salvo spec futura. |
...
|---|---|
| Mapa de specs | As 35 specs ativas têm backlog proprietário: 24 concluídas, P8-02 parcialmente entregue, P9-05 bloqueada, P10-01 concluída e P10-02 em validação após o código entregue no issue 0035; seis slices P10 permanecem pendentes. O índice corrige as referências P7/P8 a PDF; esse estado corresponde ao código, salvo as caixas históricas de P1/P7 e o DoD ainda aberto de P8-02. |
...
**Próxima passagem: validação.** Executar a matriz Chrome/Firefox/Edge do P10-02; depois detalhar P10-03..08 sobre o shell e a fundação concluídos. P9-05 continua aberto e bloqueado até resolver as decisões externas e o gate de backup físico.
