...
| Estado geral | P0–P6, P7-01/P7-02/P7-03, P8-01/P8-03, P9-01/P9-02/P9-03 concluídos; P8-02 tem slice inicial, integração Admin-only do status de backup e drill-downs de coleta, documentos, empresas e certificados concluídos nos issues 0018/0025–0029; jobs, P9-04/P9-05 seguem pendentes. |
...
|---|---|---|
| P8-02 dashboard | Slice inicial (`/api/dashboard` e `#dashboard`) é calculate-on-read, com períodos comparáveis, RBAC e capacidades indisponíveis explícitas. O issue 0025 integra o status seguro P9-02 no ramo Admin-only, os issues 0026–0028 entregam os drill-downs reconciliados de coleta, documentos e empresas, e o issue 0029 entrega o inventário reconciliado dos três cards de certificado, todos com owner canônico, filtro equivalente, total bounded, redaction e leituras sem escrita; jobs seguem indisponíveis/pendentes. A spec ainda deixa DoD posterior aberto. | Não marcar a spec/fase inteira concluída. O issue 0030 permanece para o slice de jobs; cada incremento deve manter owner, frescura, filtro equivalente e reconciliação. |
...
| 2 | **P8-02 expansões — in progress por incrementos** | O incremento de saúde de backup foi concluído no issue 0025, os drill-downs de coleta, documentos e empresas nos issues 0026–0028 e o inventário de certificados no issue 0029: `GET /api/collections/executions`, `GET /api/documents`, `GET /api/companies?lifecycle=...` e `GET /api/certificates/inventory?filter=...` usam owners canônicos, filtros allowlisted, totais reconciliados, páginas redigidas, RBAC e leituras sem escrita. Habilitar cada capacidade restante somente quando a fonte possuir owner, estado/frescura, filtro equivalente e teste de reconciliação. | P5–P7, rendering e disco continuam indisponíveis; o issue 0030 é o slice restante de drill-down; não criar snapshot/cache sem decisão documentada na spec. |
