...
| Estado geral | P0–P6, P7-01/P7-02/P7-03, P8-01/P8-03, P9-01/P9-02/P9-03 concluídos; P8-02 tem slice inicial, integração Admin-only do status de backup e drill-downs de coleta, documentos e empresas concluídos nos issues 0018/0025–0028; certificados e jobs, P9-04/P9-05 seguem pendentes. |
...
|---|---|---|
| P8-02 dashboard | Slice inicial (`/api/dashboard` e `#dashboard`) é calculate-on-read, com períodos comparáveis, RBAC e capacidades indisponíveis explícitas. O issue 0025 integra o status seguro P9-02 no ramo Admin-only, o issue 0026 entrega o drill-down de execuções dos cinco cards de coleta, o issue 0027 o drill-down reconciliado dos sete cards de documentos e o issue 0028 o drill-down reconciliado dos dois cards de empresas, todos com owner canônico, filtro equivalente, total bounded, redaction e leituras sem escrita; certificados e jobs seguem indisponíveis/pendentes. A spec ainda deixa DoD posterior aberto. | Não marcar a spec/fase inteira concluída. Issues 0029–0030 fecham os slices restantes; cada incremento deve manter owner, frescura, filtro equivalente e reconciliação. |
...
| 2 | **P8-02 expansões — in progress por incrementos** | O incremento de saúde de backup foi concluído no issue 0025, os drill-downs de coleta e documentos nos issues 0026/0027 e o drill-down de empresas no issue 0028: `GET /api/collections/executions`, `GET /api/documents` e `GET /api/companies?lifecycle=...` usam owners canônicos, filtros allowlisted, totais reconciliados, páginas redigidas, RBAC e leituras sem escrita. Habilitar cada capacidade restante somente quando a fonte possuir owner, estado/frescura, filtro equivalente e teste de reconciliação. | P5–P7, rendering e disco continuam indisponíveis; issues 0029–0030 são os próximos slices de drill-down; não criar snapshot/cache sem decisão documentada na spec. |
