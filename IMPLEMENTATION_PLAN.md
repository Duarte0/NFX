...
| 4 | **P10 Frontend UX & Visual System — in progress (P10-01/P10-02 concluídos)** | Modernizar a experiência operacional sem reescrita funcional: identidade institucional, shell desktop/notebook, componentes compartilhados e apresentação consistente dos fluxos já entregues. Specs P10-01..08 definem os contratos; cada slice só conclui com compatibilidade de contratos e validação dos estados críticos aplicáveis. | É independente de P9-05; P10-03..08 seguem depois do shell. A matriz Docker sintética de P10-02 é reproduzível sem dados reais. Preserva `App → features → shared`, contratos HTTP, IDs/âncoras, URLs de drill-down e autorização server-side; não introduzir router, estado global, framework UI ou dependência relevante sem spec e evidência. |
...
|---|---|
| Mapa de specs | As 35 specs ativas têm backlog proprietário: 25 concluídas, P8-02 parcialmente entregue, P9-05 bloqueada, P10-01/P10-02 concluídas e P10-03..08 pendentes. O índice corrige as referências P7/P8 a PDF; esse estado corresponde ao código, salvo as caixas históricas de P1/P7 e o DoD ainda aberto de P8-02. |
...
- **P10 adotado:** vinho, cinza e branco são a direção visual de produto; P10-01 registrou os valores exatos, pares de contraste e primitives em sua spec. P10-02 acrescentou o shell desktop/notebook e a navegação por papel com hashes e destino de certificado único; a matriz Docker sintética em Chrome/Firefox/Edge, com foco, teclado e larguras desktop, passou. O trabalho preserva a arquitetura `App → features → shared`, sem router, estado global, framework UI ou backend adicional por conveniência visual.
...
**Próxima passagem: P10-03..07.** Detalhar os slices de domínio sobre o shell e a fundação concluídos; P10-08 fará a validação transversal após esses slices. P9-05 continua aberto e bloqueado até resolver as decisões externas e o gate de backup físico.
