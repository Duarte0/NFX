...
| 2 | **P8-02 expansões — in progress por incrementos** | O incremento de saúde de backup foi concluído no issue 0025, os drill-downs de coleta, documentos e empresas nos issues 0026–0028, o inventário de certificados no issue 0029, o drill-down de jobs no issue 0030 e o gate cross-owner no issue 0042: `GET /api/collections/executions`, `GET /api/documents`, `GET /api/companies?lifecycle=...`, `GET /api/certificates/inventory?filter=...` e `GET /api/jobs/observability?filter=...` usam owners canônicos, filtros allowlisted, totais reconciliados, páginas redigidas, RBAC e leituras sem persistência própria do dashboard. Habilitar cada capacidade futura somente quando a fonte possuir owner, estado/frescura, filtro equivalente e teste de reconciliação. | P5–P7 e disco continuam indisponíveis; não criar snapshot/cache sem decisão documentada na spec. |
...
| 4 | **P10 Frontend UX & Visual System — concluído no issue 0041 (P10-01..P10-08)** | Modernizar a experiência operacional sem reescrita funcional: identidade institucional, shell desktop/notebook, componentes compartilhados, apresentação consistente e gate transversal de acessibilidade/responsividade/UX dos fluxos já entregues. Specs P10-01..08 têm evidência de compatibilidade de contratos e validação dos estados críticos aplicáveis. | É independente de P9-05. O issue 0041 adicionou contrato transversal, foco seguro de dialogs, redaction, landmarks, labels e métricas de clipping/overflow; a matriz Docker passou 342 testes em Chrome/Firefox/Edge a 1024/1280/1440 px. Preserva `App → features → shared`, contratos HTTP, IDs/âncoras, URLs de drill-down e autorização server-side; não introduziu router, estado global, framework UI, backend, migration ou dependência relevante. |
...
| **P10-02 Application Shell — concluído no issue 0035** | Sidebar, header, identidade do usuário, navegação, landmarks, skip link e composição principal para desktop/notebook, com preservação das áreas, hashes, callbacks e visibilidade por papel existentes. | P10-01 comprovado; matriz Docker sintética Chrome/Firefox/Edge em 1024/1280/1440 px passou. P10-03..08 seguem após o shell. Router e estado global continuam fora do escopo salvo spec futura. |
...
|---|---|
| Mapa de specs | As 35 specs ativas têm backlog proprietário: P8-02 parcialmente entregue, P9-05 bloqueada e P10-01..P10-08 concluídas. O índice corrige as referências P7/P8 a PDF; esse estado corresponde ao código, salvo as caixas históricas de P1/P7 e o DoD ainda aberto de P8-02. |
...
- **P10 adotado:** vinho, cinza e branco são a direção visual de produto; P10-01 registrou os valores exatos, pares de contraste e primitives em sua spec. P10-02 acrescentou o shell desktop/notebook e a navegação por papel com hashes e destino de certificado único; P10-03 acrescentou a composição agrupada do dashboard e a retenção stale/error do payload owner-provided; P10-04 acrescentou a apresentação de documentos com filtros/deep links, cursor opaco, estados fiscais explícitos, detalhe e ações XML/PDF owner-provided; P10-05 acrescentou a composição de empresas, certificados e coletas com estados semânticos, filtros canônicos, retenção stale/error e guards de concorrência/interação; P10-06 acrescentou a apresentação durável das exportações ZIP, sem progresso inferido, com redaction, stale/error/retry e guards de interação; as matrizes Docker sintéticas em Chrome/Firefox/Edge, com foco, teclado e larguras desktop, passaram. O trabalho preserva a arquitetura `App → features → shared`, sem router, estado global, framework UI ou backend adicional por conveniência visual.
...
**Próxima passagem:** manter as expansões incrementais de P8-02 conforme specs e owners disponíveis. P9-05 continua aberto e bloqueado até resolver as decisões externas e o gate de backup físico; P10-08 está concluído sem reabrir owners funcionais.
