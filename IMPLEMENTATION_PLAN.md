...
| 4 | **P10 Frontend UX & Visual System — in progress (P10-01..P10-05 concluídos)** | Modernizar a experiência operacional sem reescrita funcional: identidade institucional, shell desktop/notebook, componentes compartilhados e apresentação consistente dos fluxos já entregues. Specs P10-01..08 definem os contratos; cada slice só conclui com compatibilidade de contratos e validação dos estados críticos aplicáveis. | É independente de P9-05; P10-06..08 seguem após empresas, certificados e coletas. As matrizes Docker sintéticas de P10-02..P10-05 são reproduzíveis sem dados reais. Preserva `App → features → shared`, contratos HTTP, IDs/âncoras, URLs de drill-down e autorização server-side; não introduzir router, estado global, framework UI ou dependência relevante sem spec e evidência. |
...
|---|---|
| Mapa de specs | As 35 specs ativas têm backlog proprietário: 28 concluídas, P8-02 parcialmente entregue, P9-05 bloqueada, P10-01/P10-02/P10-03/P10-04/P10-05 concluídas e P10-06..08 pendentes. O índice corrige as referências P7/P8 a PDF; esse estado corresponde ao código, salvo as caixas históricas de P1/P7 e o DoD ainda aberto de P8-02. |
...
- **P10 adotado:** vinho, cinza e branco são a direção visual de produto; P10-01 registrou os valores exatos, pares de contraste e primitives em sua spec. P10-02 acrescentou o shell desktop/notebook e a navegação por papel com hashes e destino de certificado único; P10-03 acrescentou a composição agrupada do dashboard e a retenção stale/error do payload owner-provided; P10-04 acrescentou a apresentação de documentos com filtros/deep links, cursor opaco, estados fiscais explícitos, detalhe e ações XML/PDF owner-provided; P10-05 acrescentou a composição de empresas, certificados e coletas com estados semânticos, filtros canônicos, retenção stale/error e guards de concorrência/interação; as matrizes Docker sintéticas em Chrome/Firefox/Edge, com foco, teclado e larguras desktop, passaram. O trabalho preserva a arquitetura `App → features → shared`, sem router, estado global, framework UI ou backend adicional por conveniência visual.
...
**Próxima passagem: P10-06..07.** Detalhar os slices de exportações e administração sobre o dashboard, documentos, empresas, certificados, coletas, shell e fundação concluídos; P10-08 fará a validação transversal após esses slices. P9-05 continua aberto e bloqueado até resolver as decisões externas e o gate de backup físico.
