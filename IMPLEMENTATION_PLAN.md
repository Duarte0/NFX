...
|---|---|---|
| P8-02 dashboard | Slice inicial (`/api/dashboard` e `#dashboard`) é calculate-on-read, com períodos comparáveis, RBAC e capacidades indisponíveis explícitas. O issue 0025 integra o status seguro P9-02 no ramo Admin-only, e o issue 0026 entrega o drill-down de execuções dos cinco cards de coleta com total reconciliado, período `[from,to)` e redaction; P5–P7, rendering, disco e os demais drill-downs seguem indisponíveis/pendentes. A spec ainda deixa DoD posterior aberto. | Não marcar a spec/fase inteira concluída. Issues 0027–0030 fecham os outros card slices; cada incremento deve manter owner, frescura, filtro equivalente e reconciliação. |
...
| 2 | **P8-02 expansões — in progress por incrementos** | O incremento de saúde de backup foi concluído no issue 0025 e o drill-down de coleta no issue 0026: `GET /api/collections/executions` usa `CollectionExecution`, filtro civil `[from,to)`, estados allowlisted, total reconciliado e página redigida sem escrita. Habilitar cada capacidade restante somente quando a fonte possuir owner, estado/frescura, filtro equivalente e teste de reconciliação. | P5–P7, rendering e disco continuam indisponíveis; issues 0027–0030 são os próximos slices de drill-down; não criar snapshot/cache sem decisão documentada na spec. |
