...
type Certificate = { id: string; state: string; status: string; fingerprint_sha256: string; certificate_cnpj: string; not_before: string; not_after: string; days_until_expiry: number | null; key_version: number };
...
function App() {
  ...
  async function loadCompanies() {
    ...
    try {
      ...
      setCompanies(payload.companies); setSelectedCompany(payload.companies[0] ?? null); setEditCompany(payload.companies[0] ? { cnpj: payload.companies[0].cnpj, legal_name: payload.companies[0].legal_name } : { cnpj: "", legal_name: "" }); if (payload.companies[0]) void loadCertificate(payload.companies[0].id);
  ...
  async function loadCertificate(companyId: string) {
    const response = await fetch(`/api/companies/${companyId}/certificate`, { credentials: "same-origin" });
    if (!response.ok) { setCertificate(null); return; }
    const payload = (await response.json()) as { certificate: Certificate | null }; setCertificate(payload.certificate);
  }

  async function uploadCertificate(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!selectedCompany || !certificateFile || !certificatePassword) { setMessage("Selecione um arquivo .pfx e informe a senha."); return; }
    const form = new FormData(); form.append("certificate", certificateFile); form.append("password", certificatePassword);
    const response = await fetch(`/api/companies/${selectedCompany.id}/certificate/upload`, { method: "POST", credentials: "same-origin", headers: { "X-CSRFToken": cookie("csrftoken") }, body: form });
    setCertificatePassword(""); setCertificateFile(null);
    if (!response.ok) { setMessage((await response.json() as { detail?: string }).detail ?? "Não foi possível cadastrar o certificado."); return; }
    setMessage("Certificado validado e armazenado com segurança."); await loadCertificate(selectedCompany.id); await loadCompanies();
  }
